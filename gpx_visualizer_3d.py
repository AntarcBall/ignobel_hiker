import os
import matplotlib
# Try to set a GUI backend if available, otherwise use non-interactive backend
try:
    # Check if DISPLAY environment variable is set (X11 availability)
    if os.environ.get('DISPLAY'):
        matplotlib.use('TkAgg')  # Use GUI backend if display is available
    else:
        matplotlib.use('Agg')  # Use non-interactive backend
except:
    # If there's an issue with TkAgg, fallback to Agg
    matplotlib.use('Agg')
